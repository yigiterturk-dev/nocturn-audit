 ...
 * targets.json'da alan eksikse doldurur.
 */
...
/**
 * targets.json okur, yolları genişletir, eksik stack'i keşfeder.
 * Dosya yoksa boş liste döner (uyarı CLI'da verilir).
 */
export function loadRegistry(targetsPath: string): Project[] {
  ...
  try {
    ...
    throw new Error(
      `targets.json okunamadı (${targetsPath}): ${
        err instanceof Error ? err.message : String(err)
      }`,
    );
  }
  if (!raw.projects || !Array.isArray(raw.projects)) {
    throw new Error("targets.json 'projects' dizisi içermeli.");
  }
