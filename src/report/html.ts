...
function findingHtml(f: Finding): string {
  ...
  const confBadge = `<span class="confbadge ${conf === "certain" ? "certain" : "olasi"}">${
    conf === "certain" ? "CERTAIN" : "LIKELY"
  }</span>`;
...
function projectHtml(r: ProjectReport): string {
  ...
  const c = r.counts;
  const chips = `
    <span class="chip crit">${c.critical} critical</span>
    <span class="chip high">${c.high} high</span>
    <span class="chip med">${c.medium} medium</span>
    <span class="chip low">${c.low} low</span>
    <span class="chip info">${c.info} bilgi</span>`;

  // WHAT COULD NOT BE MEASURED — the most honest part of the report. A rule that
  // never ran must not share the same silence as "clean" on the same screen.
  const gaps = r.gaps.length
    ? `<details class="bosluk"><summary>Not measured: ${r.gaps.length} rule(s) did not run</summary><ul>${r.gaps
        .map((b) => `<li><b>${esc(b.ruleTitle)}</b> — ${esc(b.reason)}</li>`)
        .join("")}</ul></details>`
    : "";
  ...
  return `<article class="project">
    <header class="phead">
      ...
      <div class="pmeta">
        ...
        <span class="tag ${r.project.owned ? "owned" : "notowned"}">${r.project.owned ? "owned (live probes)" : "static-only"}</span>
    ...
    ${catSections || '<p class="clean">✓ No findings in this project.</p>'}
  </article>`;
}
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .bosluk { font-size: .8rem; color: #b9a06a; margin: .5rem 0 0; }
  .bosluk summary { cursor: pointer; }
  .bosluk ul { margin: .3rem 0 0; padding-left: 1.1rem; color: #99a; }
  ...
  .confbadge.certain { background: #3b0d0d; color: #fca5a5; border-color: #7f1d1d; }
  .confbadge.olasi { background: #1c1c28; color: #99a; border-color: #33334a; }
  .stat.certain .n { color: #f87171; } .stat.olasi .n { color: #9aa; }
...
<body>
  <div class="wrap">
    <h1>nocturn-audit</h1>
    <p class="sub">OWASP Top 10 (2021) security audit report · ${esc(dateLabel)} · ${reports.length} project(s) · detection and reporting only</p>
    <div class="summary">
      <div class="stat crit"><div class="n">${totals.critical}</div><div class="l">Critical</div></div>
      <div class="stat high"><div class="n">${totals.high}</div><div class="l">High</div></div>
      <div class="stat med"><div class="n">${totals.medium}</div><div class="l">Medium</div></div>
      <div class="stat low"><div class="n">${totals.low}</div><div class="l">Low</div></div>
      <div class="stat"><div class="n">${totals.info}</div><div class="l">Bilgi</div></div>
      <div class="stat"><div class="n">${totals.score}</div><div class="l">Risk score</div></div>
    </div>
    <div class="summary confsummary">
      <div class="stat kesin"><div class="n">${certain.critical + certain.high}</div><div class="l">CERTAIN critical+high</div></div>
      <div class="stat kesin"><div class="n">${certain.medium}</div><div class="l">CERTAIN medium</div></div>
      <div class="stat olasi"><div class="n">${heuristic.critical + heuristic.high}</div><div class="l">LIKELY critical+high</div></div>
      <div class="stat olasi"><div class="n">${heuristic.medium}</div><div class="l">LIKELY medium</div></div>
    </div>
    <p class="conflegend">Red numbers are <strong>CERTAIN</strong> (deterministic) findings; <em>LIKELY</em> findings are heuristic and need manual verification.</p>
    ${projects}
    <footer>nocturn-audit · detection only · no exploitation, no DoS · live probes run only against owned:true targets</footer>
  </div>
</body>
</html>`;
}
