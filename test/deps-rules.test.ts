...
describe("A06 — outdated-deps (deprecated list, offline)", () => {
  it("BAD fixture: deprecated paket (request + node-sass) → low/olası bulgu", async () => {
    const { ctx, cleanup } = depsCtx({
      ...
      // node_modules yok → npm outdated adımı atlanır (offline, deterministik)
    });
    try {
      ...
      expect(req.confidence).toBe("olası");
  ...
  it("CLEAN fixture: güncel/bakımlı paketler → bulgu YOK", async () => {
    const { ctx, cleanup } = depsCtx({
      "package.json": JSON.stringify({
        name: "x",
...
describe("A06 — evaluateOutdated (majors-behind, saf)", () => {
  ...
  it("1 major geride → gürültü, işaretlenmez", () => {
    const out = evaluateOutdated(
  ...
  it("current yoksa declared range'ten major çıkarılır", () => {
    const out = evaluateOutdated(
      { foo: { latest: "6.0.0" } },
      { foo: "^2.0.0" },
    );
  ...
  it("latest git/geçersiz → atlanır (FP yok)", () => {
    const out = evaluateOutdated(
